...
void worker(boost::fibers::unbuffered_channel<Search> &channel,
  ...
  while (channel.pop(search) == boost::fibers::channel_op_status::success) {
    ...
    std::vector<Finder> finders(last_move - moves,
                                Finder(start_time, search, stop));

    const auto is_stopping = [start_time, &search, &stop](int depth) {
      return depth > 1 &&
             (std::chrono::steady_clock::now() - start_time > search.time ||
              stop.load(boost::memory_order_relaxed));
    };

    Move best_move;
    unsigned long long nodes = 0;
    int max_game_ply = search.target_depth == -1
                           ? NHISTORY
                           : (search.pos.game_ply + search.target_depth);

    for (int depth = 1;
         !is_stopping(depth) && search.pos.game_ply + depth <= max_game_ply &&
         depth <= 256;
         depth++) {
      ...
      for (Move *move_ptr = moves; move_ptr != last_move; move_ptr++) {
        Move move = *move_ptr;
        tasks.push_back(pool.schedule(
            [&mtx, &prophets, us, &tts, depth, &current_best_move,
             &current_best_move_score, &current_best_move_static_evaluation,
             move](void *data) {
              mtx.lock();
              ...
              if ((prophet_it = prophets.find(boost::this_thread::get_id())) !=
                  prophets.end()) {
              ...
              static_evaluation =
                  DYN_COLOR_CALL(evaluate_nnue, us, finder->search.pos);
              if ((entry_it =
                       finder->search.rt.find(finder->search.pos.get_hash())) !=
                      finder->search.rt.end() &&
                  entry_it->second + 1 == 3) {
                score = 0;
              } else {
                bool repetition = false;
                Move nested_moves[218];
                Move *last_nested_move = DYN_COLOR_CALL(
                    finder->search.pos.generate_legals, ~us, nested_moves);
                for (Move *nested_move = nested_moves;
                     nested_move != last_nested_move; nested_move++) {
                  RT::const_iterator nested_entry_it;
                  DYN_COLOR_CALL(finder->search.pos.play, ~us, *nested_move);
                  if ((nested_entry_it = finder->search.rt.find(
                           finder->search.pos.get_hash())) !=
                          finder->search.rt.end() &&
                      nested_entry_it->second + 1 == 3) {
                    repetition = true;
                ...
                if (repetition) {
                  ...
                  score = -DYN_COLOR_CALL(finder->alpha_beta, ~us,
                                          -piece_values[KING],
                                          piece_values[KING], depth - 1);
                }
              }
              ...
              if (!finder->is_stopping()) {
                ...
                } else if (score == current_best_move_score &&
                           static_evaluation >
                               current_best_move_static_evaluation) {
                  current_best_move = move;
                  current_best_move_score = score;
                  current_best_move_static_evaluation = static_evaluation;
                }
      ...
      if (!is_stopping(depth)) {
        ...
        std::vector<Move> pv =
            get_pv(search.pos,
                   finders[std::find(moves, last_move, best_move) - moves].tt);
        ...
        std::transform(pv.cbegin(), pv.cend(), std::back_inserter(pv_strings),
                       uci::format_move);
        ...
        unsigned long long nps =
            (nodes / std::max(time_elapsed.count(), 1l)) * 1000;
        ...
        if (current_best_move_score >= piece_values[KING] - NHISTORY) {
          args = {"depth",
                  ...
                  "mate",
                  std::to_string(
                      (piece_values[KING] - current_best_move_score + 1) / 2),
                  "nodes",
                  std::to_string(nodes),
                  "time",
                  std::to_string(time_elapsed.count()),
                  "nps",
                  std::to_string(nps),
                  "pv"};
          ...
          args = {"depth",
                  ...
                  "mate",
                  std::to_string(
                      -(current_best_move_score + piece_values[KING]) / 2),
                  "nodes",
                  std::to_string(nodes),
                  "time",
                  std::to_string(time_elapsed.count()),
          ...
          args = {"depth",
                  ...
                  "score",
                  "cp",
                  std::to_string(current_best_move_score),
                  "nodes",
                  std::to_string(nodes),
                  "time",
                  std::to_string(time_elapsed.count()),
...
int main() {
  ...
  std::cout << "Orca NNUE (mono-accumulator 1x768 feature space i16 quantized "
               "eval with 64x scaling factor) compiled @ "
            << ORCA_TIMESTAMP << " on compiler " << ORCA_COMPILER << std::endl;
#endif
  ...
  boost::thread worker_thread(
      std::bind(worker, std::ref(channel), std::ref(stop)));
  ...
  for (;;) {
    ...
    str_switch(message.command) {
      ...
      str_case("eval") : str_case("evaluate") : {
        if (pos.turn() == WHITE) {
          uci::send_message("evaluation",
                            {std::to_string(evaluate<WHITE>(pos, true))});
        } else if (pos.turn() == BLACK) {
          uci::send_message("evaluation",
                            {std::to_string(evaluate<BLACK>(pos, true))});
        } else {
          throw std::logic_error("Invalid side to move");
        }
        break;
