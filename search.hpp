...
Move find_best_move(uci::Engine* engine, Position& pos, DurationT search_time, int starting_depth, tp::ThreadPool& pool, int* best_move_score_ret = nullptr, int* best_move_depth_ret = nullptr) {
    ...
    std::thread deepening_thread([engine, &pos, starting_depth, &pool, &moves, last_move, &tts, &killer_move_lists, &move_scores, &best_move, &best_move_score, &best_move_depth, &stop]() {
        ...
        for (int current_depth = starting_depth; !stop && current_depth < 256; current_depth++) {
            ...
            for (Move* move = moves; move != last_move; move++) {
                tasks.push_back(pool.schedule([pos, &moves, &tts, &killer_move_lists, &move_scores, current_depth, move, &current_mtx, &current_best_move, &current_best_move_score, &stop](void*) mutable {
                    ...
                    if (move_scores[move - moves] == -piece_values[KING] * 2 - 1) {
                        score = -alpha_beta<~Us>(pos, -piece_values[KING] * 2, piece_values[KING] * 2, current_depth - 1, tt, killer_moves, stop);
                    } else {
                        // Aspiration windows
                        int upper_window_size = 25;
                        int lower_window_size = 25;
                        for (;;) {
                            score = -alpha_beta<~Us>(pos, move_scores[move - moves] - lower_window_size, move_scores[move - moves] + upper_window_size, current_depth - 1, tt, killer_moves, stop);
                            if (score <= move_scores[move - moves] - lower_window_size) {
                                lower_window_size *= 5;
                            } else if (score >= move_scores[move - moves] + upper_window_size) {
                                upper_window_size *= 5;
                            } else {
                                break;
                            }
